...
func main() {
	...
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  SKILL.md - Claude Code skill for manual /tldt invocations")
	fmt.Fprintln(os.Stderr, "    - Installs to: ~/.claude/skills/tldt/SKILL.md")
	fmt.Fprintln(os.Stderr, "    - Allows: /tldt <long text to summarize>")
	fmt.Fprintln(os.Stderr, "    - Returns: token savings + extractive summary")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  tldt-hook.sh - Auto-trigger hook for Claude Code UserPromptSubmit")
	fmt.Fprintln(os.Stderr, "    - Installs to: ~/.claude/hooks/tldt-hook.sh")
	fmt.Fprintln(os.Stderr, "    - Auto-summarizes prompts exceeding threshold (default: 2000 tokens)")
	fmt.Fprintln(os.Stderr, "    - Runs: --sanitize --detect-injection --detect-pii before summarization")
	fmt.Fprintln(os.Stderr, "    - Output guard: re-runs detection on summary before adding to context")
	fmt.Fprintln(os.Stderr, "    - Configurable via ~/.tldt.toml [hook] threshold = N")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  Install both with: tldt --install-skill")
	fmt.Fprintln(os.Stderr, "")
